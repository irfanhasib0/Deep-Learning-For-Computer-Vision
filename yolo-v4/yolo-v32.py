...
def train_batch(image_data,target,frozen=False):
    ...
    for ind in range(len(gradients1)):
        if ind < MTL_NO_OF_SHARED_LAYERS:
            ...
            if MTL_USE_SIG_PROB:
                p_x = tf.nn.sigmoid(p_x - 0.5)
                p_y = tf.nn.sigmoid(p_y - 0.5)
            
            if MTL_USE_MUL_PROB: 
                gradients1[ind] = (alpha + (1-alpha)*p_x*p_y) * (gradients1[ind] + gradients2[ind])
                
            elif MTL_USE_IND_PROB:
                gradients1[ind] = (alpha + (1-alpha)*p_x) * gradients1[ind] + (alpha + (1-alpha)*p_y) * gradients2[ind]
                
            elif MTL_USE_COMB_PROB:
                grad_1 = (alpha + (1-alpha)*p_x*p_y) * (gradients1[ind] + gradients2[ind])
                grad_2 = (alpha + (1-alpha)*p_x) * gradients1[ind] + (alpha + (1-alpha)*p_y) * gradients2[ind]
                gradients1[ind] = 0.5 * grad_1 + 0.5 * grad_2
            else : 
