...
class YoloModel():
    def __init__(self,model=YOLO_MODEL,input_size=YOLO_INPUT_SIZE,training=False,N=1):
    ...
    def mobilenet_v2(self):
        ...
        N = int(16/self.N)
        self.backbone_model = MobileNetV2(img_shape=(YOLO_INPUT_SIZE,YOLO_INPUT_SIZE,3),
                    input_tensor=None,
                    alpha=1.0,
                    weights='imagenet',
                    pooling=None,
        ...
        self.backbone_model.build(input_shape=(None,YOLO_INPUT_SIZE,YOLO_INPUT_SIZE,3))
        self.backbone_model.set_wts()
        
    
    def shufflenet_v2(self,input_tensor):
    ...
    #@tf.function
    def yolo_head(self, NUM_CLASS=80):
        # After the input layer enters the Darknet-19 network, we get 2 branches
        self.input_tensor = tf.keras.layers.Input((YOLO_INPUT_SIZE,YOLO_INPUT_SIZE,3))
        route_1 , conv= self.backbone_model(self.input_tensor)
        ...
        self.conv_tensors = [conv_mbbox, conv_lbbox]
    ...
    def create_yolo(self,input_size=416, channels=3, training=False):
        ...
        if TRAIN_YOLO_TINY:
            ...
            if self.model == 'TINY':
                conv_tensors = self.yolov3_tiny(input_layer,NUM_CLASS)
        ...
        output_tensors = []
        for i, conv_tensor in enumerate(self.conv_tensors[:2]):
            pred_tensor = self.decode(conv_tensor, NUM_CLASS, i)
            if training: output_tensors.append(conv_tensor)
            output_tensors.append(pred_tensor)
    
        YoloV3 = tf.keras.Model(self.input_tensor, output_tensors+self.conv_tensors[2:])
        return YoloV3
    ...
    def load_yolo_model(self):
        yolo = self.create_yolo(input_size=self.input_size,training=self.training)
        '''
        if not self.training : 
            if YOLO_CUSTOM_WEIGHTS == False:
                print("Loading Darknet_weights from:",YOLO_V3_TINY_WEIGHTS)
                self.load_yolo_weights(yolo, YOLO_V3_TINY_WEIGHTS) # use Darknet weights
            else:
