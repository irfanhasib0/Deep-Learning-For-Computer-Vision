...
def generate_fake_cor(img):
    print("Generate fake correspondence files ")
    #fd = open(out_path,"w")
...
def orb_matching(img1_path, img2_path, img1_id, img2_id):
    ...
    for i, m_n in enumerate(matches):
        if len(m_n) != 2:
            ...
            ret += [[kp1[m_n[0].queryIdx].pt[0], kp1[m_n[0].queryIdx].pt[1], kp2[m_n[0].trainIdx].pt[0], kp2[m_n[0].trainIdx].pt[1], m_n[0].distance]]
    ret = np.array(ret)
    if len(ret)==0:
        return generate_fake_cor(img1)
    return ret
