...
def extract_keypoints(heatmap, total_keypoint_num):
    all_keypoints = []
    ...
    #heatmap_peaks = np.uint8(heatmap == heatmap.max())
    #print(len(_heatmap_peaks),len(heatmap_peaks))
    #if heatmap_peaks.all(): return 0
    xy_peaks  = np.nonzero(heatmap_peaks)
    keypoints = list(zip(xy_peaks[1], xy_peaks[0]))  # (w, h)
    ...
    return keypoint_num,all_keypoints,heatmap_peaks
